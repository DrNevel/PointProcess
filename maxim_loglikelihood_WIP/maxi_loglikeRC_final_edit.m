function [thetap, k, Loglikel] = maxi_loglikeRC_final_edit(xn, wn)
...
%         incr=1e-10;
        ...
        while(step<10) 
            ...
            f=sum(log(IG)-log(1-CIG)) - sum((IG ./ (1-CIG)));
            for i=1:length(update)
                ...
                f_i=sum(log(IG)-log(1-CIG)) - sum((IG ./ (1-CIG)));
                ...
                for j=1:length(update)
                    ...
                    f_j=sum(log(IG)-log(1-CIG)) - sum((IG ./ (1-CIG)));
                    
                    update_incr_ij=update+h_i;
                    update__incr_ij=update_ij+h_j;
                    
                    CIG=CumIG(wn,update_ij(end),mu(update_ij(1:end-1),xn));
                    IG=InvGauss(wn,mu(update_ij(1:end-1),xn),update_ij(end));
                    
                    f_incr_ij=sum(log(IG)-log(1-CIG)) - sum((IG ./ (1-CIG)));
                                    
                    HESS(i,j)= f_incr_ij - f_incr_i - f_incr_j + 2*f - f_decr_ij - f_decr_i - f_decr_j;
