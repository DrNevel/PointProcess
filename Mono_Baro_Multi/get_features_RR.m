function [Features,RR_correction,Coherences] = get_features_RR(selected_wf, varargin)
...
get_Compl = 0;
get_spectra = 0;
RR_correction =[]; % applies citi-barbieri correction for ectopic beats
Coherences = [];
PPorderMono = 9;
W=90;
UndSampl = 1;
...
% checks user inputs and when present substitutes the default values
...
% Data Extraction
try
    S = load(selected_wf); %% selected_wf è il filepath da caricare
    fn = fieldnames(S);
    if isstruct(S.(fn{1})) %% checks if there is a nested struct in the struct: if that's the case, substitutes S with the inner struct
        if isfield(S.(fn{1}),'annotations') %% chiedere a Max se le annotazioni sono solo gli indici e i tempi dei battiti, e nel caso sia così: cos'è la prima riga?
            S=S.(fn{1});
        end
    end
% %     checks if there is a Sig field: if that's the case, substitutes S
% %     with Sig (WHY??? first in the given cases there are no Sig fields; second why substitute a struct with a single field?)
    if isfield(S,'Sig') %% chiedere a Max se bisogna generalizzare: tra le struct date nessuna ha un campo Sig, ma campi coi nomi dei vari segnali
        S = S.Sig;
    end
%     checks for a field 'fs' or 'Fs' from struct 'S' and imposes it as the sampl. freq.
%     variable fs
    if isfield(S,'Fs')
        fs = S.Fs;
    elseif isfield(S,'fs')
    ...
    if size(S.annotations,1)==1
        EKGR = S.annotations(1,:)./fs; %% is the first row the series of indexes of R peaks?? N.W.: from this point on EKGR is a series of times, not indexes
    elseif size(S.annotations,1)>=7
        EKGR = S.annotations(sel_EKGR,:)./fs; %% sel_EKGR seems to be a selector for multiple R peaks acquisitions, as it defaults to 1 (the first acquisition)
    end
...
[EKGR,~,~,~,RR_correction] = closeECG(EKGR); % % outputs a R peaks series (EKGR) corrected for wrong beats
                                             % % it is a sort of Hampel but
                                             % % corrects on the basis of
                                             % % physio. range rather than
                                             % % STD
TACO = diff(EKGR); %% tachogram as "derivative" of R peaks event series

if rsmpl_ts
    TACO_ts = timeseries(TACO,EKGR(2:end));
    resfreq = 1/mean(TACO); %% equivalent sample freq of non regularly sampled time series
    % Resampling Using Re-sampled R-peaks Times
    taco_times = EKGR(2):resfreq:EKGR(end); %% shouldn't the time step be 1/resfreq (= mean(TACO))??
    TACO_ts = resample(TACO_ts,taco_times,'linear'); %% resamples TACO_ts at taco_times (regular sampling), assuming linear interpolation at times with no data
    TACO_res = squeeze(TACO_ts.Data)'; %% TACO_res(ampled)
else
    taco_times = EKGR(2:end); %% from the second as only the second element make sense as a time difference
    TACO_res = TACO; %% if no resampling is required, makes the "input" tachogram the resampled one
end
% %% TO VISUALIZE
...
% Extract Features
% PointProcess
if get_PointProcess %% generates point process parameters
        ...
        if get_spectra %% generates spectral parameters only if also PP is requested (spectra are generated from PP parameters)
            [Features.PP.Mono.RR.powLF, Features.PP.Mono.RR.powHF, Features.PP.Mono.RR.bal,...
                warn, Features.PP.Mono.RR.powVLF, Features.PP.Mono.RR.hrv_ind.powTot,Features.PP.Mono.RR.S_RR,...
                Features.PP.Mono.RR.FS] =...
                hrv_indices(Features.PP.Mono.Thetap, Features.PP.Mono.s2, 1./Features.PP.Mono.meanRR, fres);
            [Features.PP.Mono.RR.lbl, Features.PP.Mono.RR.outs] = ...
                MonoBaro(Features.PP.Mono.Thetap, PPorderMono, Features.PP.Mono.s2, 1./Features.PP.Mono.meanRR, fres, 1:9);
        end
...
if get_TimeDomain
    ...
    Features.CLASS.NN20=[]; %% the amount of NN that exceed by 20 ms the previous one
    Features.CLASS.NN50=[]; %% the amount of NN that exceed by 50 ms the previous one
