...
for i=1:length(COV) % COV{i} is assumed to have same length of EKGR
    ...
    ck_v = [ck_v,tmp(:)]; % values
end

% events in obs window
cc = ck_v(1:lastRi-1,:);
...
% perchè Thetap_Cov init qui invece che solo dopo if mono_cov_flg?
% P2: order of monovariate AR models on COV only (def:[]) --> Thetap_Cov
% coeff of COV only 
Mu_Gaus = NaN(1, J);
Cov_Gaus = NaN(1, J);
...
for j = ceil(W / delta):J
    ...
    if event
        ...
        lastRi = lastRi + 1;        
        if isempty(opt.Rsim) %Rsim line 72
            R = EKGR(lastRi);
        else
    ...
    if isempty(thetap)
        ...
        for i=1:length(COV) % COV{i} is assumed to have same length of EKGR
            clear wn2
            % Extract Positions for Mu regression - Searching the closer Mu value for covariates regression
            [~,min_ix] = min(abs(time_ax-cc_t(1)));
            [~,max_ix] = min(abs(time_ax-cc_t(end)));
            % min(A,[],2): column vector containing the minimum value of each row.
            [~,mu_cc_t(:,i)] = min(abs(time_ax(min_ix:max_ix)-cc_t),[],2);
            wn2(:,i) = Mu(mu_cc_t(:,i)+min_ix);
            clear mu_cc_t
        end
        wn2 = wn2(P+1:end,:); % cos'è wn2?
        %disp(wn);
    ...
    meanCOV(j) = eta' * wcn / sum(eta);
    opt.LogLikel(:,j) = sum(loglikel);
    % Gauss Params
    Mu_Gaus(j) = thetap2(:,1)'*xt; % RR Bivariate series Gaussian
    Cov_Gaus(j) = thetap2(:,2)'*xt; % COV Bivariate series Gaussian
    Thetap2(:,:,j) = thetap2;
    Var2(:,j) = var2;
    if mono_cov_flg
        % Cov Params
