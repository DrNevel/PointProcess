function [KSdistance,Z,T,ordered,KSoutPerc,lin,lu,ll] = ks_plot(EKGR, L, delta, plt)
...
for j = first:length(L)+1
    ...
    if event
        ...
        intL = intL + dt * pr;
        T(lastRi) = intL; % Integral of the Hazard-Rate
        intL = 0;
    elseif j<=length(L)
        intL = intL + delta * L(j);
    end
...
Z = 1 - exp(-T); % Time-Rescaling Theorem and Exponential Transformation to extract quantiles
Z = Z(~isnan(Z));
...
KSoutPerc = sum(ordered<ll|ordered>lu)./length(ordered);
